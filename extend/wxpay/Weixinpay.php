...
class Weixinpay extends controller {
    ...
    public function __construct(){
        ...
        if (empty($this->config)) {
            $this->config=config('WEIXINPAY_CONFIG');
        }else{
            $this->config=$config;
        }
    }
...
//  	echo $dingdan['name'];
        ...
        if (!isset($_GET['code'])) {
            ...
            // 返回的url
            $redirect_uri=url('Paynotify/pay',0,'',TRUE);
            ...
            // 订单数据  请根据订单号out_trade_no 从数据库中查出实际的body、total_fee、out_trade_no
            $order_info = $this->get_order_info($out_trade_no);

            $order=array(
                'body'=>'宜惠乐商城',// 商品描述（需要根据自己的业务修改）
                'total_fee'=>$order_info['total_fee'] * 100,// 订单金额  以(分)为单位（需要根据自己的业务修改）
    ...
    /**
     * 订单信息查询
     * @param  $out_trade_no 订单号 有可能是充值订单号，也有可能是商品订单号
     * @param  返回订单的总价格，订单的描述
     */
    public function get_order_info($out_trade_no){
        //首先查询订单号是否存在于商品订单中
        $g_order = db('order')->where('order_number',$out_trade_no)->field('total,order_id')->find();
        if($g_order){
            $order['total_fee'] = round($g_order['total'],2);
            $order['order_id'] = $g_order['order_id'];
            return $order;
        }else{
            $c_order = db('recharge')->where('recharge_number',$out_trade_no)->field('money,status,recharge_id')->find();
            if($c_order){
                $order['total_fee'] = round($c_order['money'],2);
                $order['order_id'] = $c_order['recharge_id'];
                return $order;
            }
        }
    }   
